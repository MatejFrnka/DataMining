...
public class Jabeja {
  ...
  private void sampleAndSwap(int nodeId) {
    ...
    if (config.getNodeSelectionPolicy() == NodeSelectionPolicy.HYBRID
      ...
      partner = findPartner(getNeighbors(nodep), nodeId);
    }
    ...
    if (config.getNodeSelectionPolicy() == NodeSelectionPolicy.HYBRID
      ...
      if (partner == null)
        partner = findPartner(getSample(nodeId), nodeId);
    }
  ...
  public Node findPartner(Integer[] nodes, int nodeId) {
    ...
    for (int node : nodes) {
      ...
      double current = 0, acceptanceProb = 0;
